...
function get_build_release_candidate() {
  ...
  _ZM_BUILD_RELEASE_WITHOUT_BETA="${_ZM_BUILD_RELEASE_NO_WITH_PATCH%.[bB][eE][tT][aA]}"
  if [ "${_ZM_BUILD_RELEASE_WITHOUT_BETA}" == "${_ZM_BUILD_RELEASE_NO}" ] ; then
    _ZM_BUILD_RELEASE_SANITIZED=""
  else
...
BUILD_RELEASE_CANDIDATE="$(get_build_release_candidate ${ZM_BUILD_RELEASE_NO_WITH_PATCH} ${ZM_BUILDER_ID} ${ZM_BUILD_RELEASE_NO})"
